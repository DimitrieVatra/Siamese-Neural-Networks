...
class MNIST(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        if self.train:
            ...
            self.support_set = np.stack([self.transform(Image.fromarray(self.data[x].numpy(), mode='L')) for x in indices])   

    def __getitem__(self, index):
        image1_index = np.random.randint(0, len(self.data))
        image1 = self.data[image1_index]
        target1 = int(self.targets[image1_index])

        image2 = self.data[index]
        target2 = int(self.targets[index])
